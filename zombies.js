...
function Player(name, health, strength, speed) {
  ...
  this.addToPack = function(item) {
    pack.push(item);
  };
  this.removeFromPack = function(index) {
    pack.splice(index, 1);
  };
...
Player.prototype.checkPack = function() {
  console.log(this.getPack().map(function(item) {
    return item.name;
  }));
}
...
Player.prototype.takeItem = function(item) {
  var pack    = this.getPack(),
      output  = '',
      result;
  if (!(item instanceof Item) && !(item instanceof Food) && !(item instanceof Weapon)) {
    result = false;
  } else if (pack.length >= 3) {
    output = 'The pack is full so the item could not be stored';
    result = false;
  } else {
    this.addToPack(item);
    output = this.name+', '+item.name;
  }
  console.log(output);
  return result;
}
...
Player.prototype.discardItem = function(item) {
  ...
  if (!(item instanceof Item) && !(item instanceof Food) && !(item instanceof Weapon)) {
    ...
    index = pack.indexOf(item);
    if (index < 0) {
      output = 'Nothing was discarded since the '+item.name+' could not be found.';
      result = false;
    } else {
      this.removeFromPack(index);
      output = this.name+', '+item.name+' was discarded';
      result = true;
    }
  }
  console.log(output);
...
Player.prototype.equip = function(itemToEquip) {
  ...
  if (!(itemToEquip instanceof Weapon)) {return false;}

  pack = this.getPack();
  index = pack.indexOf(itemToEquip);

  if (this.equipped) {
    this.removeFromPack(index);
    this.addToPack(this.equipped);
...
Player.prototype.eat = function(itemToEat) {
  ...
  if (!(itemToEat instanceof Food)) { return false;}

  pack = this.getPack();
  index = pack.indexOf(itemToEat);

  if (index >= 0) {
    this.removeFromPack(index);
    health = this.health + itemToEat.energy;
    this.health = (health > this.getMaxHealth()) ? this.getMaxHealth() : health;
  }
}
...
Player.prototype.useItem = function(item) {
  ...
  else if (item instanceof Food) {this.eat(item);}
}
